...
class DeductionSplit:
    def __init__(self, invoice_s):
        self.invoice_s = invoice_s
    # def __init__(self, datasets, invoice_number_s, object_id, accident_no, paid_amount, deductible_amount):
    #     deductible_amount_sdatasets = datasets
    #     deductible_amount_sinvoice_numbers = invoice_number_s
    #     deductible_amount_sobject_id = object_id
    #     deductible_amount_saccident_no = accident_no
    #     deductible_amount_spaid_amount = paid_amount
    #     deductible_amount_sdeductible_amount = deductible_amount
    #     deductible_amount_scomplex_fields = list(zip(object_id, accident_no, paid_amount, deductible_amount))
    #     pass
    def computed(self, invoice_number_s, deductible_amount_s, accident_no_s):
        if sum(deductible_amount_s) == sum(invoice_number_s):
            max_deductible_amount = max(deductible_amount_s)
            max_invoice_numbers = max(invoice_number_s)
            if max_deductible_amount > max_invoice_numbers:

                excess_deductible_amount = max_deductible_amount - max_invoice_numbers

                deductible_amount_s.remove(max_deductible_amount)    # 移除面额最大的 待打包项
                invoice_number_s.remove(max_invoice_numbers)    # 移除面额最大的 发票


                # 移除上述2个面额 等价宇 一对一拆分
                # obj_deduction = PartialDeduction(invoice_info=datasets, accident_no=split_accident_no, deduction_amount=max_deductible_amount)
                # obj_deduction.partial_deduction()

                deductible_amount_s.append(excess_deductible_amount)
                self.computed(invoice_number_s, deductible_amount_s, accident_no_s)
            # else:
            #     print('已确保所有待打包金额没有超过发票最大金额')
            #     deductible_amount_s = sorted(deductible_amount_s, reverse=True)
            #     invoice_number_s = sorted(invoice_number_s, reverse=True)
            #
            #     compensate_deductible_amount = max_invoice_numbers - max_deductible_amount
            #
            #     compensate_accumulation = []
            #
            #     for deductible_amount_ in deductible_amount_s:
            #         compensate_accumulation.append(deductible_amount_)
            #
            #         deductible_amount_s.remove(deductible_amount_)
            #         if sum(compensate_accumulation) >= compensate_deductible_amount:
            #             if sum(compensate_accumulation) != compensate_deductible_amount:
            #                 deductible_amount_s.append(sum(compensate_accumulation) - compensate_deductible_amount)
            #             invoice_number_s.remove(max_invoice_numbers)  # 移除面额最大的 发票
            #             break
        else:
            print('打包总金额不等于发票总金额')
        return invoice_number_s, deductible_amount_s
    
    def split_deductible_amount(self, complex_fields, invoice_number_s):
        object_id_s, accident_no_s, paid_amount_s, deductible_amount_s = [], [], [], []
        for object_id_, accident_no_, paid_amount_, deductible_amount_ in complex_fields:
            object_id_s.append(object_id_)
            accident_no_s.append(accident_no_)
            paid_amount_s.append(paid_amount_)
            deductible_amount_s.append(deductible_amount_)
            pass
        return self.computed(invoice_number_s, deductible_amount_s, accident_no_s)
        
                # split_deductible_amount()
